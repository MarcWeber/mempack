...
import { defaultResolveImplementation, dependencyTree, CTX } from "../../src/dependencies";
...
const graph_of = async (entryPoint: string) => {
  const foobar = path.resolve(process.cwd(), "foobar")
  fs.writeFileSync(foobar, "1", "utf8")
  console.log("1", (await gS.cache.get("filetype")(foobar).p))
  console.log("2", (await gS.cache.get("filetype")(foobar).p))
  gS.cache.invalidatePath(foobar)
  fs.writeFileSync(foobar, "2", "utf8")
  console.log("3", (await gS.cache.get("filetype")(foobar).p))
  ...
  for (const v of [1, 2, 3, 4, 5, 6]) {
    l.push(`\n kind ${v}\n`)
    const transpiled = await gS.cache.get("transpiled")({ moduleKind: v, path: p }).p
    // const transpiled = ts.transpileModule(fs.readFileSync(file.path, "utf8"), { compilerOptions: { module: v } })
    l.push(transpiled.outputText)
    l.push("\n===\n")
  }
  ...
  try {
    ...
    for (const file of g.files()) {
      ...
      for (const v of [1, 2, 3, 4, 5, 6]) {
        lines.push(`\n kind ${v}\n`)
        const transpiled = await gS.cache.get("transpiled")({moduleKind: v, path: file.path}).p
        // const transpiled = ts.transpileModule(fs.readFileSync(file.path, "utf8"), { compilerOptions: { module: v } })
